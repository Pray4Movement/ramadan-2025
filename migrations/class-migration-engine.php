 ...
 */
class DT_Ramadan_2025_Migration_Engine
{
    /**
        ...
        foreach ( $filenames as $filename ) {
            if ( $filename[0] !== '.' && $filename !== 'abstract.php' && $filename !== 'class-migration-engine.php' ){
                if ( preg_match( '/^([0-9][0-9][0-9][0-9])(-.*)?\.php$/i', $filename, $matches ) ) {
                    ...
                    $migration_name = sprintf( 'DT_R2025_Migration_%04d', $got_migration_number );
     ...
     *
     * @throws \Exception ...
     * @throws \DT_R2025_Migration_Lock_Exception ...
     * @throws \Throwable ...
     */
    public static function migrate( int $target_migration_number ) {
        ...
        while ( true ) {
            $current_migration_number = get_option( 'dt_r2025_migration_number' );
            ...
            if ( (int) get_option( 'dt_r2025_migration_lock', 0 ) ) {
                throw new DT_R2025_Migration_Lock_Exception();
            }
            update_option( 'dt_r2025_migration_lock', '1' );

            error_log( gmdate( ' Y-m-d H:i:s T' ) . " Starting migrating to number $activating_migration_number" );
            try {
                $migration->up();
            } catch ( Throwable $e ) {
                update_option( 'dt_r2025_migrate_last_error', [
            ...
            update_option( 'dt_r2025_migration_number', (string) $activating_migration_number );
            error_log( gmdate( ' Y-m-d H:i:s T' ) . " Done migrating to number $activating_migration_number" );

            update_option( 'dt_r2025_migration_lock', '0' );
...
class DT_R2025_Migration_Lock_Exception extends Exception
{
    public function __construct( $message = null, $code = 0, Exception $previous = null ) {
         ...
         */
        $last_migration_error = get_option( 'dt_r2025_migrate_last_error' );
        if ( $message === null ) {
